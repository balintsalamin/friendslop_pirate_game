...
var is_at_helm := false
@onready var _ship = get_tree().get_first_node_in_group("ship")
...
func _on_camera_perspective_changed(first_person: bool) -> void:
	if not is_multiplayer_authority():
		return
	_body.visible = true
	_bottom_mesh.visible = true
	_chest_mesh.visible = true
	_face_mesh.visible = true
...
func _physics_process(delta):
	...
	is_at_helm = _ship != null and _ship.driver_peer_id == multiplayer.get_unique_id()
	if Engine.get_physics_frames() % 30 == 0:
		print("my_id=%s driver=%s is_at_helm=%s" % [multiplayer.get_unique_id(), _ship.driver_peer_id if _ship else "NO SHIP", is_at_helm])

	if Input.is_action_just_pressed("interact"):
		if is_at_helm:
			_request_exit_helm()
		elif is_on_floor() and _get_helm_in_front():
			_request_enter_helm()
...
func _update_body_facing() -> void:
	_body.rotation.y = wrapf(_spring_arm_offset.rotation.y + PI, -PI, PI)  # +PI: kompenzáljuk a SpringArmOffset beépített 180°-os elforgatását
func _move() -> void:
...
func _get_helm_in_front() -> Node:
	var pickup_area := get_node_or_null("GodotRobot3D/InfrontArea3D") as Area3D
	if not pickup_area:
		return null
	for body in pickup_area.get_overlapping_bodies():
		if body.is_in_group("helm"):
			return body
	return null


func _request_enter_helm() -> void:
	if not _ship:
		return
	_ship.request_enter_helm.rpc_id(1)


func _request_exit_helm() -> void:
	if not _ship:
		return
	_ship.request_exit_helm.rpc_id(1)
